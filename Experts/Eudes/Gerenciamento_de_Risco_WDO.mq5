//+------------------------------------------------------------------+
//|         Boleta_WDO_Com_Painel_v3.39.mq5                          |
//|         Adaptado de Boleta_Indice_Com_Painel_v3.35.mq5 (WIN)     |
//+------------------------------------------------------------------+
...
// Tabela de Fases baseada na imagem de referência (WDO - Minidólar)
...
double ObterPontosAlvosEfetivos(double &slOut, double &tpOut, bool &ehOperacaoBonus)
{
   ...
   {
      double lftAtual = CalcularLFT(faseAtual, E, papeisPorOperacao);
      double valorPorPontoPorLote = 10.0; // R$ por ponto por papel (WDO - mesma constante das fórmulas de Acúmulo/LFT)
...
//+------------------------------------------------------------------+
//| Fórmulas de LFT / Acúmulos (WDO) - dependem da fase e da          |
//| quantidade de negócios (E) configurada/permitida para o dia.      |
//| Diferente do WIN, aqui o multiplicador de papéis usado nas        |
//| fórmulas é o Lote Max FIXO da fase (TabelaFases[fase].loteMax),   |
//| e não a quantidade de papéis (V) escolhida pelo usuário - o       |
//| parâmetro "papeis" é mantido apenas por compatibilidade de        |
//| assinatura com os pontos de chamada existentes.                   |
//+------------------------------------------------------------------+

// Acúmulo progressivo = (nº de negócios) * (nº máximo de papéis da fase) * (pontos de gain da fase) * 10
double CalcularAcumProg(int fase, int negociosDia, int papeis)
{
   return negociosDia * TabelaFases[fase].loteMax * TabelaFases[fase].gainPontos * 10.0;
}

// Acúmulo regressivo = (nº de negócios) * (nº máximo de papéis da fase) * (pontos de loss da fase) * 10
double CalcularAcumReg(int fase, int negociosDia, int papeis)
{
   return negociosDia * TabelaFases[fase].loteMax * TabelaFases[fase].lossPontos * 10.0;
}

// LFT = ((quantidade de negócios do dia / 2) arredondado para BAIXO) * (nº máximo de papéis da fase) * 20 * ((pontos de loss da fase * -1) / 2)
double CalcularLFT(int fase, int negociosDia, int papeis)
{
   int floorOrdens2 = (int)MathFloor(negociosDia / 2.0);
   return floorOrdens2 * TabelaFases[fase].loteMax * 20.0 * ((TabelaFases[fase].lossPontos * -1.0) / 2.0);
}
...
void EnviarOrdemMercado()
{
   ...
   if(ordemOk)
   {
      if(PositionSelect(_Symbol))
      {
         ...
         {
            slFinal = NormalizeDouble(precoExecucao - pontosSL, _Digits);
            tpFinal = NormalizeDouble(precoExecucao + pontosTP, _Digits);
         }
         else
         {
            slFinal = NormalizeDouble(precoExecucao + pontosSL, _Digits);
...
void EnviarOrdemPendente(double pontosSL, double pontosTP)
{
   ...
   if(tipoOperacao == 1) // compra
   {
      ...
      sl = NormalizeDouble(precoAlvo - pontosSL, _Digits);
      tp = NormalizeDouble(precoAlvo + pontosTP, _Digits);
...
void DefaultAtualizarLinhasCustomizadas()
{
   ...
   if(tipoOperacao == 1)
   {
      ...
      slProjetado     = precoReferencia - pontosSL;
      tpProjetado     = precoReferencia + pontosTP;
...
void AtualizarPainelVisualEmTempoReal()
{
   ...
   string strExSL = DoubleToString(exSL, 1); StringReplace(strExSL, ".", ",");
   string strExTP = DoubleToString(exTP, 1); StringReplace(strExTP, ".", ",");
   string textoFase = StringFormat("Fase %d - SL: %s pts | TP: %s pts", faseAtual, strExSL, strExTP);
